...
class _ReviewQueueScreenState extends ConsumerState<ReviewQueueScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await ref.read(clinicalRepositoryProvider).listDrafts();
      if (mounted) {
        setState(() {
          _drafts = rows;
          if (_selected != null) {
            ClinicalDraft? refreshed;
            for (final item in rows) {
              if (item.id == _selected!.id) refreshed = item;
            }
            _selected = refreshed;
          }
        });
  ...
  Future<void> _review(ClinicalDraft draft, String status) async {
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              status == 'approved'
                  ? 'Approval records your independent clinician review. Verify source facts, dangerous alternatives, missing data, and the proposed next step.'
    ...
    try {
      final updated = await ref.read(clinicalRepositoryProvider).review(
            draftId: draft.id,
            status: status,
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'Confirm source-fact fidelity, uncertainty, dangerous alternatives, missing information, and the proposed next step. Do not approve solely because a draft is fluent.',
                    ),
