...
class NuraMedicalReleaseApp extends ConsumerWidget {
  const NuraMedicalReleaseApp({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return MaterialApp(
      ...
      home: auth.loading
          ...
          : auth.user == null
              ? const LoginScreen()
              : NuraReleaseHome(user: auth.user!),
    );
  }
...
class NuraReleaseHome extends StatefulWidget {
  const NuraReleaseHome({super.key, required this.user});

  final AppUser user;

  @override
  State<NuraReleaseHome> createState() => _NuraReleaseHomeState();
}

class _NuraReleaseHomeState extends State<NuraReleaseHome> {
  int _tab = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final selected = _tab >= 0 && _tab < screens.length ? _tab : 0;
    return Scaffold(
