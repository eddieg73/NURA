...
class Settings(BaseSettings):
    ...
    demo_reviewer_email: str = "clinicalreview@nuratech.ai"
    demo_reviewer_password: str = "ReplaceBeforeRelease123!"
    ...
    def validate_for_startup(self) -> None:
        if self.app_env == "production":
            if len(self.jwt_secret) < 32 or "change-me" in self.jwt_secret.lower():
                raise RuntimeError("JWT_SECRET must be a unique 32+ character secret in production")
            if self.seed_demo_data:
                raise RuntimeError("SEED_DEMO_DATA must be false in production")
            if "*" in self.allowed_origins or "localhost" in self.allowed_origins:
                raise RuntimeError("ALLOWED_ORIGINS must contain only approved production origins")
        ...
        if self.ai_provider == "openai":
            ...
            if not (self.openai_baa_confirmed and self.openai_phi_approved):
                raise RuntimeError(
                    "OpenAI clinical routing is blocked until BAA and PHI approval flags are true"
                )
