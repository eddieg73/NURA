...
def create_seed_user(db: Session, organization: Organization, email: str, password: str, role: str) -> None:
    ...
    if db.scalar(select(User).where(User.email == normalized)) is None:
        db.add(
            User(
                ...
                email=normalized,
                full_name="App Review Clinician" if role == "clinician" else "NURA Administrator",
...
def seed_demo(db: Session) -> None:
    ...
    create_seed_user(db, organization, settings.admin_email, settings.admin_password, "admin")
    db.commit()
...
app = FastAPI(
    ...
    version="1.0.0",
    description=(
        "Clinician-facing draft generation, workflow, review, and account API. "
...
def export_account(
    ...
    drafts = db.scalars(
        select(ClinicalDraft).where(ClinicalDraft.organization_id == user.organization_id)
    ).all()
    tasks = db.scalars(select(OpsTask).where(OpsTask.user_id == user.id)).all()
    ...
    return {
        ...
        "clinical_drafts": [draft_out(item).model_dump(mode="json") for item in drafts],
...
async def create_clinical_draft(
    ...
    result.output.provenance.append(
        {"source_type": "encounter", "source_id": encounter.id, "note": "User-entered source text"}
    )
    draft = ClinicalDraft(
        organization_id=user.organization_id,
