...
Currently configured project: `nura_medical` (target: `..`).
...
original init command. Checkpoints in `data/nura_medical/setup/setup-state.json`
...
`--project-id nura_medical` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/nura_medical/`.
