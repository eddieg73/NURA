# Agent setup for `nura_medical`

This is a deepsec scanning workspace. Project `nura_medical` was just registered
(target: `..`). Setup is incomplete — `data/nura_medical/INFO.md`
...
2. **Fill in `data/nura_medical/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id nura_medical
pnpm deepsec process --project-id nura_medical
```
